  ...
  if (hit) {
    ...
    */
    if (renderable->refractance() > 0 &&
        incident_ray.num_reflections() < max_num_reflections) {
      double eta = renderable->index_of_refraction();
      vec3 N = hit->normal;
      vec3 I = incident_ray.direction();
      double cosi = -dot(N, I);
      double k = 1 - eta * eta * (1 - cosi * cosi);
      vec3 T = k < 0 ? vec3::zeros() : eta * I + (eta * cosi - sqrt(k)) * N; // refraction direction
      Ray refracted_ray(hit->position - 1e-6 * hit->normal, T, incident_ray.num_reflections() + 1);
      vec3 refracted_color = shade_closest_intersection(refracted_ray, 1e-6);
